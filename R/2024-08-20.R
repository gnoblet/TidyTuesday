...
older <- dyr$filter(df, age_diff < 0) |> nrow()
tot <- nrow(df)
...
gg$ggplot(df) + 
  ...
  gg$annotate(
    ...
    y = max - 3,
    label = sgr$str_wrap(paste0(max.which.king.name, " was ", abs(max), " years older than consort ", max.which.consort.name, ". "), 35),
    color = "#ca562c",
    size = 4.5,
    halign = 0.5
  ) +
  gg$annotate(
    ...
    yend = max - 0.5,
    color = "#ca562c",
    arrow = gg$arrow(length = gg$unit(0.01, "npc")), curvature = -0.1) +
  gg$annotate(
    geom = "text",
    x = min.which.year - 250,
    y = min - 3,
    label = sgr$str_wrap(paste0(min.which.king.name, " was ", abs(min), " years younger than consort ", min.which.consort.name), 30),
    color = "#3d5941",
    size = 4.5,
    halign = 0.5
  ) +
  ...
  # See minus 10 tick add limits to 20
  ...
  gg$theme(
    ...
    plot.title = ggt$element_markdown(size = 20),
    # increase size of subtitle
    plot.subtitle = ggt$element_markdown(size = 18)
  ) 
# Save svg
gg$ggsave("2024-08-20.svg", plot = last_plot(), width = 30, height = 18, units = "cm")
