---
title: "Gallery"
subtitle: "All TidyTuesday viz in one gallery, organized by year"
format: 
  html:
    page-layout: full
    toc: false
...
if (nrow(all_metadata) == 0) {
  ...
  for (i in 1:nrow(all_metadata)) {
    ...
    if (file.exists(image_path) || file.exists(abs_image_path)) {
      ...
      cat('<a href="', row$relative_path, '" style="color: white; text-decoration: none;">', row$title, '</a>\n')
      cat('</div>\n\n')
    }
    
    # Title
    cat("**[", row$title, "](", row$relative_path, ")**  \n")
